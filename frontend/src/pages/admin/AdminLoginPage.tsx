...
import { loginAdminThunk } from '../../redux/authThunk'
...
const AdminLoginPage = () => {
  ...
  // 1. Initialize Redux hooks
  const dispatch = useDispatch()
  const { loading, error, isAuthenticated, user } = useSelector((state: any) => state.auth);


  // Track where the user came from if intercepted by a route guard
  const from = location.state?.from

  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')

  // 2. Clear old authentication errors when the page mounts
  useEffect(() => {
    dispatch(clearError());
  }, [dispatch]);

  // 3. Monitor the Redux state for a successful login transition
  useEffect(() => {
    if (isAuthenticated && user?.role === 'admin') {
      // navigate(from && String(from).startsWith('/admin') ? from : '/admin/dashboard', {
      //   replace: true,
      // })
      navigate('/admin');
  ...
  // 4. Handle Login Submission via Redux Thunk
  const handleSubmit = (e) => {
    e.preventDefault();

    // Dispatch the payload directly to your async thunk
